...
TEST_F(ASPRCC8, Department)
{
    ...
    EXPECT_TRUE(satified) << "The StudentArea should be externallyConnected to mainHallA) and disconnected to mainHallB).";

    EXPECT_TRUE(queryObject->getHeadValues().size() == 2) << "The query should create 2 headValues but contains " << queryObject->getHeadValues().size() << ".";

    EXPECT_TRUE(queryObject->getLifeTime() == 0) << "The query should be expired but lifetime is:" << queryObject->getLifeTime() << ".";

    EXPECT_TRUE(((reasoner::ASPSolver*) aspSolver->getSolver())->getRegisteredQueriesCount() == 0)
            << "There shouldn't be any query left but there are " << ((reasoner::ASPSolver*) aspSolver->getSolver())->getRegisteredQueriesCount() << " left.";
