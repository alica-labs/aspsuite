...
{
    shared_ptr<map<Clingo::Symbol, vector<Clingo::Symbol>>> ret = make_shared<map<Clingo::Symbol, vector<Clingo::Symbol>>>();
...
void ASPFactsQuery::onModel(Clingo::Model& clingoModel)
{
    ...
    //		for(auto it = ((ASPSolver*)this->solver)->clingo->out_->predDoms().begin(); it != ((ASPSolver*)this->solver)->clingo->out_->predDoms().end();
    //it++)
    //		{
