...
void PredicateContainer::rememberPredicates(const std::vector<SyntaxUtils::Predicate>& predicates)
{
    for (auto& predicate : predicates) {
        this->rememberPredicate(predicate.name, predicate.arity);
    }
}

void PredicateContainer::rememberPredicate(std::string predicateName, int arity)
{
...
bool PredicateContainer::lookUpPredicate(const std::string& predicateName, int arity)
{
    if (predicatesToAritiesMap.find(predicateName) != predicatesToAritiesMap.end()) {
        // name matched
...
std::vector<SyntaxUtils::Predicate> PredicateContainer::determineHeadPredicates(const std::string& rule)
{
    ...
    while (!done) {
        ...
        semicolonPair =
                std::pair<SyntaxUtils::Separator, size_t>(SyntaxUtils::Separator::Semicolon, SyntaxUtils::findNextChar(rule, ";", implicationIdx, currentIdx));
        ...
        case SyntaxUtils::None:
            ...
            break;
        default:
            std::cerr << "determineHeadPredicates: Separator Char not known!" << std::endl;
        }
