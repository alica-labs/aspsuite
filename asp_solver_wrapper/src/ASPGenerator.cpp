...
std::string ASPGenerator::inRefPlanTaskState(std::string prefix, const Condition* c, std::string plan, std::string task, std::string state, bool dotTerminated)
{
    return "inRefPlanTaskState(" + get(std::move(prefix), c) + ", " + plan + ", " + task + ", " + state + (dotTerminated ? ")." : ")");
}
