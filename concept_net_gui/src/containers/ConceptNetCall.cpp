...
//		this->connect(this, SIGNAL(nextAdjective()), this, SLOT(findAdjectives()));
	...
	void ConceptNetCall::findAdjectives()
	{
		for (auto edge : this->edges)
		{
			...
			{
				if (std::find(this->adjectives.begin(), this->adjectives.end(), edge->firstConcept->term.toStdString())
						== this->adjectives.end())
				{
					this->adjectives.push_back(edge->firstConcept->term.toStdString());
				}
			}
			if (edge->secondConcept->senseLabel.compare("a") == 0)
			{
				if (std::find(this->adjectives.begin(), this->adjectives.end(), edge->secondConcept->term.toStdString())
						== this->adjectives.end())
				{
					this->adjectives.push_back(edge->secondConcept->term.toStdString());
				}
			}
	...
	void ConceptNetCall::checkAdjectives()
	{

		//TODO check every adjective if there is an antony edge
		//get data
