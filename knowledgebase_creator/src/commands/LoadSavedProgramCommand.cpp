...
LoadSavedProgramCommand::LoadSavedProgramCommand(KnowledgebaseCreator* gui, QString fileName, QByteArray loadedData)
{
        ...
        for (int i = 0; i < cmds.size(); i++) {
            ...
            if (cmd["type"].toString().toStdString().compare("New Solver") == 0) {
                std::shared_ptr<NewSolverCommand> c = std::make_shared<NewSolverCommand>(
                        this->gui, std::make_shared<SolverSettings>(cmd["name"].toString().toStdString(), cmd["settingsString"].toString().toStdString()));
                c->execute();
            ...
            // Handle change settings command
                ...
                std::shared_ptr<ChangeSolverSettingsCommand> c = std::make_shared<ChangeSolverSettingsCommand>(this->gui, this->gui->settingsDialog,
                        std::make_shared<SolverSettings>(cmd["name"].toString().toStdString(), cmd["settingsString"].toString().toStdString()));
                c->execute();
                emit this->gui->updateCommandList();
                continue;
            }
            ...
            // Handle load background knowledge command
                ...
                std::shared_ptr<LoadBackgroundKnowledgeCommand> c = std::make_shared<LoadBackgroundKnowledgeCommand>(this->gui, cmd["fileName"].toString());
                c->execute();
                emit this->gui->updateCommandList();
                continue;
            }
            ...
            // Handle assignExternal command
                ...
                std::shared_ptr<AssignExternalCommand> c =
                        std::make_shared<AssignExternalCommand>(this->gui, cmd["external"].toString(), cmd["truthValue"].toString());
                c->execute();
                emit this->gui->updateCommandList();
                continue;
            }
