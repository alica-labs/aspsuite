...
void KnowledgebaseCreator::conceptNetCallBack()
{
    ...
    std::shared_ptr<ConceptNetQueryCommand> cmd = std::make_shared<ConceptNetQueryCommand>(this, this->ui->aspRuleTextArea->toPlainText());
    cmd->execute();
}
...
void KnowledgebaseCreator::applyExternalCallBack()
{
    ...
    std::shared_ptr<AssignExternalCommand> cmd =
            std::make_shared<AssignExternalCommand>(this, this->ui->externalTextEdit->text(), this->ui->truthValueComboBox->currentText());
    cmd->execute();
}
...
void KnowledgebaseCreator::connectGuiElements()
{
    ...
    this->connect(this->ui->actionLoad_Background_Knowledge, SIGNAL(triggered()), this->slHandler, SLOT(loadBackgroundKnowledge()));
