...
namespace kbcr
{
SettingsDialog::SettingsDialog(QWidget* parent, KnowledgebaseCreator* gui)
        : QDialog(parent)
...
void SettingsDialog::connectElements()
{
    ...
    this->connect(this->ui->settingsListWidget, SIGNAL(itemDoubleClicked(QListWidgetItem*)), this, SLOT(fillSettingsLabel(QListWidgetItem*)));
    this->connect(this->ui->settingsListWidget, SIGNAL(itemDoubleClicked(QListWidgetItem*)), this, SLOT(applySettings()));
...
void SettingsDialog::loadSettingsFromConfig()
{
    ...
    for (int i = 0; i < settings.size(); i++) {
        ...
        this->parameterMap.emplace(set["name"].toString().toStdString(),
                std::make_shared<SolverSettings>(set["name"].toString().toStdString(), set["parameters"].toString().toStdString()));
    }
...
void SettingsDialog::applySettings()
{
    ...
    std::shared_ptr<ChangeSolverSettingsCommand> cmd = std::make_shared<ChangeSolverSettingsCommand>(this->mainGui, this, this->currentSettings);
    cmd->execute();
    writeSettings();
    this->close();
}
...
{
    if (this->ui->paramLineEdit->text().toStdString().empty() || this->ui->nameLineEdit->text().toStdString().empty()) {
        return;
    }
    this->parameterMap.emplace(this->ui->nameLineEdit->text().toStdString(),
            std::make_shared<SolverSettings>(this->ui->nameLineEdit->text().toStdString(), this->ui->paramLineEdit->text().toStdString()));
    fillSettingsList();
}
