...
{
    auto item = new QTableWidgetItem(QString(std::dynamic_pointer_cast<NewSolverCommand>(cmd)->settings->argString.c_str()));
    item->setFlags(item->flags() ^ Qt::ItemIsEditable);
...
{
    auto item = new QTableWidgetItem(QString(std::dynamic_pointer_cast<ChangeSolverSettingsCommand>(cmd)->currentSettings->argString.c_str()));
    item->setFlags(item->flags() ^ Qt::ItemIsEditable);
    this->gui->getUi()->commandHistoryTable->setItem(pos, 1, item);
}
...
{
    auto item = new QTableWidgetItem(QString(std::dynamic_pointer_cast<GroundCommand>(cmd)->historyProgramSection.c_str()));
    item->setFlags(item->flags() ^ Qt::ItemIsEditable);
    this->gui->getUi()->commandHistoryTable->setItem(pos, 1, item);
}
...
{
    auto item = new QTableWidgetItem(QString(std::dynamic_pointer_cast<AddCommand>(cmd)->historyProgramSection.c_str()));
    item->setFlags(item->flags() ^ Qt::ItemIsEditable);
    this->gui->getUi()->commandHistoryTable->setItem(pos, 1, item);
}
...
{
    auto it = std::find_if(
            this->gui->commandHistory.begin(), this->gui->commandHistory.end(), [cmd](std::shared_ptr<Command> const& i) { return i.get() == cmd.get(); });
